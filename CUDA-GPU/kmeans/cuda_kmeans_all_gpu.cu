...
void find_nearest_cluster(int numCoords,
  ...
  if (tid < numObjs) {
    ...
    /* call min_dist = euclid_dist_2(...) with correct objectId/clusterId using clusters in shmem*/
    min_dist = euclid_dist_2_transpose(numCoords, numObjs, numClusters, deviceobjects, shmemClusters, tid, 0);

    for (i = 1; i < numClusters; i++) {
      /* call dist = euclid_dist_2(...) with correct objectId/clusterId using clusters in shmem*/
                      ...
                      double *deviceClusters)    //  [numCoords][numClusters])
{
  ...
  if (tid < total) {
    ...
    int sz = devicenewClusterSize[clusterId];
    if (sz > 0) {
      deviceClusters[coordId * numClusters + clusterId] =
          devicenewClusters[coordId * numClusters + clusterId] / (double)sz;
    }

    devicenewClusters[coordId * numClusters + clusterId] = 0.0;
