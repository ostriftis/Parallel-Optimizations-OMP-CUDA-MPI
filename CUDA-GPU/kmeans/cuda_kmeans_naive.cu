...
__device__ int get_tid() {
  return threadIdx.x + blockDim.x * blockIdx.x; /* DONE: Calculate 1-Dim global ID of a thread */
}
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  const unsigned int numClusterBlocks = (numObjs > blockSize) ? ((numObjs + numThreadsPerClusterBlock - 1) / blockSize): 1; /* DONE: Calculate Grid size, e.g. number of blocks. */
