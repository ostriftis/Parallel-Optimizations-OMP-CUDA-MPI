...
int main(int argc, char **argv)
{
     ...
     */
    kmeans(objects, numCoords, (int)rank_numObjs, numClusters, threshold, loop_threshold, membership, clusters);
    ...
    */
    ...
    int rank_numObjs_int = (int)rank_numObjs;

    MPI_Gather(&rank_numObjs_int, 1, MPI_INT, recvcounts, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (rank == 0) {
       ...
       for (i = 0; i < size; i++) {
         if (i == 0) displs[0] = 0;
	 else displs[i] = recvcounts[i-1] + displs[i-1];
       }
     ...
     */
    MPI_Gatherv(membership, (int) rank_numObjs, MPI_INT, tot_membership, recvcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
