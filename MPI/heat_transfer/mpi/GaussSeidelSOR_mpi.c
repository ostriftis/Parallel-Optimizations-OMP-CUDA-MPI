	...
	*/
    ...
    for (t=0;t<T;t++) {
		...
		if (north != MPI_PROC_NULL) {
			MPI_Sendrecv(&(u_previous[1][1]), 1, updown, north, 22,
				     &(u_previous[0][1]), 1, updown, north, 22, MPI_COMM_WORLD, &stat);
		}
		if (south != MPI_PROC_NULL) {
			MPI_Sendrecv(&(u_previous[local[0]][1]), 1, updown, south, 22,
				     &(u_previous[local[0]+1][1]), 1, updown, south, 22, MPI_COMM_WORLD, &stat);
		}
		if (east != MPI_PROC_NULL) {
			MPI_Sendrecv(&(u_previous[1][local[1]]), 1, side, east, 22,
				     &(u_previous[1][local[1]+1]), 1, side, east, 22, MPI_COMM_WORLD, &stat);
		}
		if (west != MPI_PROC_NULL) {
			MPI_Sendrecv(&(u_previous[1][1]), 1, side, west, 22,
				     &(u_previous[1][0]), 1, side, west, 22, MPI_COMM_WORLD, &stat);
		}
		/*Add appropriate timers for computation*/
		
		if (west != MPI_PROC_NULL) MPI_Recv(&u_current[1][0], 1, side, west, 17, MPI_COMM_WORLD, &stat);
		if (north != MPI_PROC_NULL) MPI_Recv(&u_current[0][1], 1, updown, north, 17, MPI_COMM_WORLD, &stat);		
		...
		gettimeofday(&tcms, NULL);
		if(east != MPI_PROC_NULL) MPI_Send(&u_current[1][local[1]], 1, side, east, 17, MPI_COMM_WORLD);
		if(south != MPI_PROC_NULL) MPI_Send(&u_current[local[0]][1], 1, updown, south, 17, MPI_COMM_WORLD);
